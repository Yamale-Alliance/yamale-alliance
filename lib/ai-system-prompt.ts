...
- **Never claim a statute "does not exist" in a country** or that Yamalé has "no" national act on a topic. If this turn's search did not attach the instrument, say the **corpus did not return it for this query** and recommend verifying with official sources (/library, country + instrument name). Spelling variants (e.g. Trade Mark vs Trademark) may still be indexed—absence in one turn is not proof of absence in the library.`
}

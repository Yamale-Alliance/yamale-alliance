  ...
  return (
    <div className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 md:py-10">
      ...
      <Tabs.Root value={tab} onValueChange={setTab} className="mt-8">
        ...
        >
          <TabTrigger value="analytics" icon={LineChart} label={t("tabs.analytics")} />
          <TabTrigger value="subscriptions" icon={CreditCard} label={t("tabs.subscriptions")} />
          <TabTrigger value="lawyer_searches" icon={Search} label={t("tabs.lawyerSearches")} />
          <TabTrigger value="library_pdf" icon={FileDown} label={t("tabs.libraryPdfs")} />
          <TabTrigger value="vault" icon={LayoutGrid} label={t("tabs.vault")} />
        </Tabs.List>
        ...
        <Tabs.Content value="vault" className="mt-8 outline-none focus-visible:ring-2 focus-visible:ring-primary/30 rounded-xl">
          <div className="rounded-2xl border border-dashed border-primary/25 bg-gradient-to-br from-primary/[0.06] to-transparent p-8 sm:p-10">
            <h3 className="text-xl font-semibold text-foreground">{t("vaultTab.title")}</h3>
            <p className="mt-2 max-w-xl text-sm text-muted-foreground">{t("vaultTab.description")}</p>
            <Link
              href="/admin-panel/marketplace"

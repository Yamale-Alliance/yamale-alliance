...
export function PurchasedLawsClient({ initialLawIds }: Props) {
  ...
  const refreshIds = useCallback(async () => {
    const res = await fetchDocumentExportUnlockLawIds();
    if (res.ok) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-muted-foreground">
                Documents you unlocked for PDF export — tied to your account in the database. Open one to preview,
                download, or print again.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        {loading ? (
          ...
          <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-8 py-10 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {lawIds.length} {lawIds.length === 1 ? "unlock is" : "unlocks are"} on your account, but the library did
              not return those documents. They may have been removed or your session may have expired.
            </p>

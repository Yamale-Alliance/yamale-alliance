...
export default function MarketplaceZipPackageLayout({ children }: { children: React.ReactNode }) {
  return (
    <div
      className={`${lfpSerif.variable} ${lfpSans.variable} min-h-screen text-foreground antialiased [--site-nav-h:4.5rem] [--vault-chrome-h:5.25rem] sm:[--site-nav-h:5.5rem] sm:[--vault-chrome-h:4.75rem]`}
    >
      {children}
    </div>
  );

    ...
    <div className="p-4 sm:p-6">
      ...
      <div className="mt-6 overflow-x-auto rounded-xl border border-border">
        {loading ? (
          ...
          <p className="px-4 py-12 text-center text-muted-foreground">{t("empty")}</p>
        ) : (
          <table className="w-full min-w-[640px] text-left text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                <th className="p-3 font-medium">{t("table.title")}</th>
                <th className="p-3 font-medium">{t("table.user")}</th>
                <th className="p-3 font-medium">{t("table.status")}</th>
                <th className="p-3 font-medium">{t("table.lastActivity")}</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-border hover:bg-muted/30">
                  ...
                  <td className="p-3 capitalize">{t(`status.${r.status}`)}</td>
